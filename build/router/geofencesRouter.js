...
const geofences_1 = require("../models/geofences");
...
class GeofencesRouter {
     ...
     */
    getOneGeofence(req, res) {
        const _id = req.params._id;
        geofences_1.default.findOne({ "_id": _id })
            .then(data => {
            res.json({
                data
            });
    ...
    getGeofences(req, res) {
        ...
        geofences_1.default.find(req.query)
            .skip(area.offset)
            .limit(area.limit)
            .sort(sortBy)
            .then(data => {
    ...
    createGeofence(req, res) {
        let geofence = null;
        try {
            geofence = new geofences_1.default(req.body);
    ...
    updateGeofence(req, res) {
        const _id = req.params._id;
        geofences_1.default.findOneAndUpdate({ _id }, req.body)
            .then((data) => {
            res.json({
                data
            });
    ...
    deleteGeofence(req, res) {
        const _id = req.params._id;
        geofences_1.default.findOneAndRemove({ _id }) //field name: value
            .then((data) => {
            res.json({
                data
            });
...
exports.default = geofencesRoutes.router;
