...
const flimmecams_1 = require("../models/flimmecams");
...
class FlimmecamsRouter {
     ...
     */
    getOneFlimmecam(req, res) {
        const _id = req.params._id;
        flimmecams_1.default.findOne({ "_id": _id })
            .then(data => {
            res.json({
                data
            });
     ...
     */
    createFlimmecam(req, res) {
        let flimmecam = null;
        try {
            flimmecam = new flimmecams_1.default(req.body);
        }
        catch (e) {
            console.log(e);
        }
     ...
     * @param res: return an status code or an error
     */
    updateFlimmecam(req, res) {
        const _id = req.params._id;
        flimmecams_1.default.findOneAndUpdate({ _id }, req.body)
            .then((data) => {
            res.json({
                data
            });
     ...
     * @param res: return an status code or an error
     */
    deleteFlimmecam(req, res) {
        const _id = req.params._id;
        flimmecams_1.default.findOneAndRemove({ _id }) //field name: value
            .then((data) => {
            res.json({
                data
            });
...
exports.default = flimmecamsRoutes.router;
