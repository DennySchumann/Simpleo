...
class ConfigsRouter {
     ...
     */
    public getOneConfig(req: Request, res: Response): void {
        const _id: String = req.params._id;

        Configs.findOne({"_id": _id})
            .then(data => {
     ...
     * @param res: return an object or error message for the client
     */
    public getConfigs(req: Request, res: Response): void {
        let sortBy = Helper.getSort(req.query);
        let area = Helper.getLimit(req.query);

        Configs.find(req.query)
            .skip(area.offset)
            .limit(area.limit)
            .sort(sortBy)
            .then(data => {
     ...
     */
    public createConfig(req: Request, res: Response): void {
        let config = null;

        try {
            config = new Configs(req.body);
    ...
    public updateConfig(req: Request, res: Response): void {
        ...
        Configs.findOneAndUpdate({_id}, req.body)
            .then((data) => {
                res.json({
                    data
                });
     ...
     * @param res: return an status code or an error
     */
    public deleteConfig(req: Request, res: Response): void {
        const _id: String = req.params._id;

        Configs.findOneAndRemove({_id}) //field name: value
            .then((data) => {
                res.json({
                    data
                });
...
export default configsRoutes.router;